...
public enum CapabilityFlags {
    CLIENT_LONG_PASSWORD(0x00000001, "Use the improved version of Old Password Authentication. Assumed to be set since 4.1.1."),
    CLIENT_FOUND_ROWS(0x00000002, "Send found rows instead of affected rows in EOF_Packet."),
    CLIENT_LONG_FLAG(0x00000004, "Longer flags in Protocol::ColumnDefinition320.Server Supports longer flags. Client Expects longer flags."),
    ...
    CLIENT_LOCAL_FILES(0x00000080, "Can use LOAD DATA LOCAL. Server Enables the LOCAL INFILE request of LOAD DATA|XML."
    ...
    CLIENT_IGNORE_SIGPIPE(0x00001000, "Client Do not issue SIGPIPE if network failures occur (libmysqlclient only)."
            + " See mysql_real_connect()"),
    CLIENT_TRANSACTIONS(0x00002000, "Server Can send status flags in EOF_Packet. Client Expects status flags in EOF_Packet."
            + " This flag is optional in 3.23, but always set by the server since 4.0."),
    CLIENT_RESERVED(0x00004000, "Unused. Was named CLIENT_PROTOCOL_41 in 4.1.0."),
    CLIENT_SECURE_CONNECTION(0x00008000, "Server Supports Authentication::Native41. Client Supports Authentication::Native41."),
